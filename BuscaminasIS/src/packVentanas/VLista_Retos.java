...
public class VLista_Retos extends JFrame{
		...
		private VLista_Retos() throws ExcepcionConectarBD, SQLException {
			...
			if(x<listaRetos.size()) {
				JLabel lblUsuario_1 = new JLabel(""+GestorUsuarios.getGestorUsuarios().getNombreUsuario(listaRetos.get(x).getUsuarioRetador()));
			...
			if(x<listaRetos.size()) {
				...
				btnAceptar4.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						try {
	...
	public void rechazar(int x) throws ExcepcionConectarBD, SQLException{
		ArrayList<Reto> listaRetos = GestorRetos.getGestorRetos().getListaRetos();
		GestorRetos.getGestorRetos().eliminarReto(listaRetos.get(x-1).getCodReto());
	}
	public void aceptar(int x) throws ExcepcionConectarBD, SQLException{
		ArrayList<Reto> listaRetos = GestorRetos.getGestorRetos().getListaRetos();
		GestorBuscaminas.getGestorBuscaminas().iniciarPartidaReto(listaRetos.get(x-1).getCodReto());
		setVisible(false);
	}
}
