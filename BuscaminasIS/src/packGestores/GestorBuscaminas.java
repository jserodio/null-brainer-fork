...
public class GestorBuscaminas {
	...
	public void guardarPartida(Partida pPartida) {
		...
		try {
			...
			// Añadir tablero
			Tablero tablero = pPartida.getTablero();
			String borrarTablero = "DELETE FROM TABLERO WHERE CODTABLERO = '" + tablero.getCodTablero() + "'";
			String sentencia2 = "INSERT INTO TABLERO (CODTABLERO,NIVEL,COLUMNAS,FILAS, LMINAS, LCASILLASVACIAS, LCASILLASPORVISITAR, LCASILLASVISITADAS, LCASILLASBANDERA, CONTADORMINAS, CASILLAS, CREADOR ) VALUES ('"
					+ tablero.getCodTablero() + "','" + tablero.getNivel() + "'," + tablero.getColumnas() + ","
					+ tablero.getFilas() + ",'" + tablero.getlMinas().toString() + "','"
					+ tablero.getCasillasVacias().toString() + "','" + tablero.getCasillasPorVisitar().toString()
					+ "','" + tablero.getCasillasVisitadas().toString() + "','"
					+ tablero.getlCasillasBandera().toString() + "'," + tablero.getlMinas().size() + ","
					+ (tablero.getFilas() * tablero.getColumnas()) + "," + pPartida.getJugador().getCodUsuario() +")";

			GestorBD.getConexionBD().actualizarBD(borrarTablero);
			GestorBD.getConexionBD().actualizarBD(sentencia2);
...
//		GestorSesion.getSesion().setContBanderas(GestorSesion.getSesion().getContMinas());
	...
	public void compartirTwitter() {
		...
		try {
			ResultSet result = GestorBD.getConexionBD()
					.consultaBD("SELECT NIVEL FROM TABLERO WHERE CODTABLERO=" + codTablero + ";");
			if (result != null) {
				result.next();
				nivel = result.getInt("NIVEL");
			}
			// MaxPuntuacion de un tablero
			ResultSet result1 = GestorBD.getConexionBD()
					.consultaBD("SELECT MAX(PUNTUACION) FROM PARTIDA WHERE CODTABLERO=" + codTablero + ";");
			if (result1 != null) {
				result1.next();
				maxPuntuacionTablero = result.getInt("MAX(PUNTUACION)");
			}
			// MaxPuntuacion de un nivel
			ResultSet result2 = GestorBD.getConexionBD().consultaBD(
					"SELECT MAX(PUNTUACION) FROM PARTIDA NATURAL JOIN TABLERO WHERE Tablero.nivel=" + nivel + ";");
			if (result2 != null) {
				result2.next();
				maxPuntuacionNivel = result.getInt("MAX(PUNTUACION)");
			}
			// MaxPuntuacion de usuario
			ResultSet result3 = GestorBD.getConexionBD()
					.consultaBD("SELECT MAX(PUNTUACION) FROM PARTIDA WHERE CODUSUARIO=" + codJugador + ";");
			if (result3 != null) {
				result3.next();
	...
	public void anadirPistas() {
		...
		try {
			GestorBD.getConexionBD().actualizarBD(
					"UPDATE USUARIO SET PISTAS=" + u.getNumPistas() + " WHERE CODUSUARIO=" + codJugador + ";");
		} catch (ExcepcionConectarBD e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
